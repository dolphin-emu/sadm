...
def _get_prod_playstore_version(play, package_name):
    edit_id = play.edits().insert(
        body={}, packageName=package_name).execute()['id']
    tracks = play.edits().tracks().list(
        editId=edit_id, packageName=package_name).execute()
    for track in tracks['tracks']:
        if track['track'] != _PLAYSTORE_TRACK:
            continue
        return track['releases'][0]['name']


def _upload_new_playstore_apk(play, package_name, apk, info):
    edit_id = play.edits().insert(
        body={}, packageName=package_name).execute()['id']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--update_track', type=str, required=True)
    parser.add_argument('--service_key_file', type=str, required=True)
    args = parser.parse_args()

    play = _get_playstore_service(args.service_key_file)

    latest_dolphin_info = _get_dolphin_update_info(args.update_track)
    playstore_version = _get_prod_playstore_version(play, args.package_name)
    ...
    _upload_new_playstore_apk(play, args.package_name, apk,
                              latest_dolphin_info)
